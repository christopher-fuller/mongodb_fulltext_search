...
The "index collections" will benefit from using MongoDB indexes themselves. To enable MongoDB indexes on the "index collections", simply use the normal rake command +rake db:mongoid:create_indexes+ (or for development or testing, you can activate the auto-creation of indexes if your ODM supports it).
