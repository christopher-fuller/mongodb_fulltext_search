...
With Mongoid for example, use the normal rake command <tt>rake db:mongoid:create_indexes</tt> (or for development or testing, you can enable auto-creation of indexes using the +autocreate_indexes+ configuration setting). With MongoMapper for example, the indexes should be created automatically by default.
