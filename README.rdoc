...
The indexes are implemented as MongoDB collections (since MongoDB's built-in indexes do not currently support fulltext search), therefore the fulltext indexes could be considered "index collections". The "index collections" are stored in the same database that the models are persisted to.
...
To blacklist words from the indexes, stop words can be specified in a YAML config file.
